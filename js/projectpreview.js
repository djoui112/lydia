...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (!projectId) {
    ...
    if (pageTitle) {
      pageTitle.textContent = 'Error: No project ID';
    }
    const projectTitle = document.getElementById('projectTitle');
    if (projectTitle) {
      projectTitle.textContent = 'Error: No project ID provided';
    }
    return;
  }
...
async function loadProjectPreview(projectId) {
  try {
    ...
    await renderPortfolioSection(can_add_to_portfolio);
    ...
    const pageTitle = document.querySelector('.page-title');
    if (pageTitle) {
      pageTitle.textContent = 'Error Loading Project';
    }
    
    const projectTitle = document.getElementById('projectTitle');
    if (projectTitle) {
      projectTitle.textContent = 'Error: ' + (error.message || 'Failed to load project');
    }
    
    const projectDescription = document.getElementById('projectDescription');
    if (projectDescription) {
      projectDescription.textContent = 'Please check the console for more details.';
    }
    
    // Show error message
    const container = document.querySelector('.container');
    if (container) {
      container.innerHTML = '<div style="padding: 40px; text-align: center;"><h2>Error Loading Project</h2><p>' + (error.message || 'Please try again later.') + '</p><button onclick="window.location.reload()" style="margin-top: 20px; padding: 10px 20px; background-color: #6A2E2A; color: white; border: none; border-radius: 4px; cursor: pointer;">Retry</button></div>';
    }
  }
}
...
function renderProjectInfo(project) {
  ...
  if (moreDetailsLink && project.id) {
    moreDetailsLink.href = `project-details.html?id=${project.id}`;
  }
...
async function renderPortfolioSection(canAddToPortfolio) {
  ...
  if (canAddToPortfolio) {
    ...
    if (addPortfolioBtn) {
      // Get architect ID from session or project data
      let architectId = window.projectPreviewData?.project?.assigned_architect_id;
      console.log('Architect ID from project:', architectId);
      ...
      if (!architectId) {
        try {
          ...
          console.error('Failed to fetch architect ID:', error);
        }
      }
      
      console.log('Final architect ID:', architectId);
      if (architectId) {
        const portfolioUrl = `architect-portfolio.html?architect_id=${architectId}`;
        addPortfolioBtn.href = portfolioUrl;
        console.log('Portfolio link set to:', portfolioUrl);
        // Also add click handler as backup
        addPortfolioBtn.onclick = function(e) {
          e.preventDefault();
          window.location.href = portfolioUrl;
        };
      } else {
        // Try to get ID on click as fallback
        addPortfolioBtn.onclick = async function(e) {
          e.preventDefault();
          try {
            const response = await fetch('../php/api/get-session-architect.php', {
              credentials: 'include'
            ...
            console.error('Failed to fetch architect ID on click:', error);
          }
          // Fallback to portfolio without ID
          window.location.href = 'architect-portfolio.html';
        };
        console.log('Portfolio link set to default (no ID) - will fetch on click');
      }
    }
  } else {
